...
class CustomCommands:
    ...
    async def learn(self, ctx, command : str, *, text):
        ...
        Example:
        !learn yourcommand Text you want
        """
    ...
    async def edit(self, ctx, command : str, *, text):
        ...
        Example:
        !edit yourcommand Text you want
        """
        server = ctx.message.server
        command = command.lower()
        if server.id in self.c_commands:
            ...
            if command in cmdlist:
                ...
                self.c_commands[server.id] = cmdlist
                dataIO.save_json(self.file_path, self.c_commands)
                await self.bot.say("i re-learned that!")
            else:
                await self.bot.say("That command doesn't exist. Use learn [command] [text]")
        else:
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def thingsiknow(self, ctx):
